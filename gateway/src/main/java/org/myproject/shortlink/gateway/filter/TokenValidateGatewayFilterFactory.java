package org.myproject.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import org.myproject.shortlink.gateway.config.Config;
import org.myproject.shortlink.gateway.dto.GatewayErrorResult;
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                Object userInfo;
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get("login_" + username, token)) != null) {
